...
Pour tout $k=1, 2, \ldots K$, on dispose de $n_k$ observations, que l'on suppose être des réalisations de $n_k$ variables aléatoires $X_{k,1}$, $X_{k,2}$, \ldots , $X_{k, n_k}$  indépendantes et de loi ${\cal N}(m_k , \sigma^2)$.

La variance $\sigma^2$ est commune à tous les groupes et est supposée inconnue.

On chercher à tester l'hypothèse nulle $H_0$: $m_1=\cdots =m_K$.

On posera dans la suite ${\bf X} = (X_{ki} , 1\leq k \leq K, 1 \leq i \leq n_k)$.

\begin{enumerate}
\item Ecrire la vraisemblance des observations ${\bf X}$.
\item Calculer l'estimateur de maximum de vraisemblance des paramètres du
  modèle sous $H_0$.
\item Calculer l'estimateur de maximum de vraisemblance des paramètres du
  modèle.
\item Montrer que le test de rapport de vraisemblance généralisé conduit à
  utiliser une statistique de test dont la loi suit une loi de Fisher dont on
  précisera les degrés de liberté.
\item Quelle est la forme de la région de rejet de $H_0$ pour ce test?
\end{enumerate}

\subsection{Un exemple}
On s'intéresse à l'impact de différents régimes alimentaires sur le poids de
chiens de même race. On dispose d'un groupe témoin (ou contrôle) de 10 animaux
qui reçoivent une alimentation classique et de deux groupes tests à qui l'on
fait suivre deux régimes différents. Les résultats (en kg) par groupe sont
...
\end{tabular}
\vspace*{0.5cm}
On vérifie alors que $\bar{x}=5.073$ et que $\sum_k \sum_i (x_{ki}
-\bar{x}_k)^2=14.2584$. Peut-on conclure que ces différents régimes
alimentaires n'ont pas tous le même effet sur le poids des animaux?
