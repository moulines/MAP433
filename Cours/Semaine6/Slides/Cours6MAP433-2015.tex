...
\end{itemize}
\begin{theo}[Existence d'un test de niveau $\alpha$]
...
\begin{frame}
\frametitle{Preuve test U.P.P.-1}
\only<1>{\begin{itemize}
\item Soit $\varphi$ un test satisfaisant les conditions
$$
\PE_{\theta_0}[\phi(Z)]= \alpha
$$
et
...
\end{cases}
\]
\item Soit  $\varphi^*$ un test de niveau $\PE_0[\varphi^*(Z)] \leq \alpha$
\end{itemize}}
\only<2>{
\begin{itemize}
\item On note:
\begin{align*}
S^+ &= \{z : \varphi(z) - \varphi^*(z) > 0\} \\ 
S^- &=\{ z: \varphi(z) - \varphi^*(z) < 0\} .
\end{align*}
\item Pour $z \in S^+$, $\phi(z) > 0$ et donc \alert{$p_1(z) \geq c_\alpha p_0(z)$} (car $\varphi(z)=0$ si $p_1(z) < c_\alpha p_0(z)$).
\item Pour $z \in S^-$, $\phi(z) < 1$ et donc \alert{$p_1(z) \leq c_\alpha p_0(z)$} (car $\varphi(z)=1$ si $p_1(z) > c_\alpha p_0(z)$).
\item Par conséquent:
\begin{multline*}
\int (\varphi - \varphi^*) (p_1 - c_\alpha p_0) \rmd \mu  \\
= \int_{S^+ \cup S^-}  (\varphi - \varphi^*) (p_1 - c_\alpha p_0) \rmd \mu \geq 0
\end{multline*}
\end{itemize}
}
\only<3>
...
\alert{
\begin{align*}
\int (\varphi - \varphi^*) p_1  \rmd \mu 
&\geq c_\alpha \int (\varphi - \varphi^*)  p_0 \rmd \mu \\
&\geq c_\alpha \left\{ \PE_{0}[\varphi(Z)] - \PE_{0}[\varphi^*(Z)]  \right\}\\
&= c_\alpha \{ \alpha - \PE_{0}[\varphi^*(Z)] \} \geq 0 \eqsp.
\end{align*}
}
}
\end{frame}
...
\begin{lem}
Notons $\puissance$ la puissance du test U.P.P de niveau $\alpha$ du test $H_0= \{\theta = \theta_0\}$ contre l'alternative $H_1= \{\theta=\theta_1\}$. Alors $\alpha \leq \puissance$ avec égalité si $\PP_{\theta_0}= \PP_{\theta_1}$.
\end{lem}
\begin{proof}
Comme le test $\varphi(z) \equiv \alpha$ a un niveau $\alpha$, nous avons $\alpha \leq \puissance$.
\end{proof}
\end{frame}
...
%$$\boxed{c_\alpha = \exp\big(n\frac{(\truetheta_1-\truetheta_0)^2}{2}+\sqrt{n}(\truetheta_1-\truetheta_0)\Phi^{-1}(1-\alpha)\big)}$$
\end{itemize}
\end{frame}
