...
Marc Hoffmann, Universit\'e Paris-Dauphine \\
%\begin{center}
{\small \hspace{3mm} hoffmann@ceremade.dauphine.fr}
%\end{center}
\vspace{4mm}

{\color{red} PC}
\begin{itemize}
\item Stéphane Gaiffas, \'Ecole Polytechnique.
\item Christophe Giraud, Université Paris-Sud et \'Ecole Polytechnique. 
\item Guillaume Lecu\'e,  \'Ecole Polytechnique.
\item Mathieu Rosenbaum, Université Pierre-et-Marie Curie.
\end{itemize}
\end{frame}
\begin{frame}
\frametitle{Organisation : agenda} {\color{red} Cours et PC
\vspace{2mm}}

Les vendredis 31 janvier, 7 février, 14 février, 21 février, 7 mars, 21 mars, 28
mars, 4 avril, 11 avril. \vspace{0.7cm}

{\color{red} Evaluation}
\begin{itemize}
\item {\color{red} Contrôle classant} : noté sur $20$.
%\item {\color{red} Participation aux PC} : noté sur 2.
\item {\color{red} Projet en binôme} : (présentés le 7 février) noté sur 4.
\end{itemize}
%\hline
...
\section{Présentation (succinte) du cours}

\begin{frame}
\frametitle{Présentation (succinte) du cours}
\begin{itemize}
...
\item {\color{red} 1 cours} d'ouverture ou de compléments {\small (en fonction de l'auditoire)}.
\end{itemize}
\end{frame}
...
\begin{frame}
c'est-à-dire
$$\boxed{\PP^X(dx) = f(x)1_{\big\{x < 1\big\}}dx + \Big( \int_{[1,+\infty)}f(u)du\Big)\delta_1(dx)}$$
\underline{Formule de calcul}
\begin{align*}
\E\big[\varphi(X)\big] &= \int_{\R}\varphi(x)\PP^X(dx)\\
& = \int_{(-\infty,1)}\varphi(x)f(x)dx+\Big(\int_{[1,+\infty)}f(u)du\Big)\varphi(1).
\end{align*}
\end{frame}
...
$$\boxed{\PP^X\big[(a,b]\big] = \PP\big[a < X \leq b\big] = F(b)-F(a)}$$
