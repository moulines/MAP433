...
\item
\begin{enumerate}
\item Solution 1: La convergence en proba de $Y_n$ vers $c$ implique que $e^{isY_n}$ converge en proba vers  $e^{isc}$. D'autre part, comme $e^{isY_n}$ est une v.a (complexe) bornée, alors  $e^{isY_n}$ converge vers  $e^{isc}$ dans $L^1$. On en conclut que le premier terme tend vers zéro.
\item Solution 2: pour tout $\delta>0$,
\begin{align*}
\mathbb{E}(|e^{isY_n}-e^{isc}|) &= \mathbb{E}(|e^{isY_n}-e^{isc}|\un_{|Y_n-c|\geq \delta}) + \mathbb{E}(|e^{isY_n}-e^{isc}|\un_{|Y_n-c|< \delta}) \\
&\leq
\end{align*}
\end{enumerate}
