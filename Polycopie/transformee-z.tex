...
\end{definition}
On appelle encore \eqref{eq:definition-transformee} la transform\'{e}e directe, car c'est la relation qui permet d'obtenir $U(z)$ \`{a} partir de $\sequence{u}[n][\zset]$ . L'op\'{e}ration inverse porte le nom de transformation inverse.
...
R_{-}<|z|<R+
$$
Ceci est illustr\'{e} par \Cref{fig:Zfigure2-2}. Lorsque $R+\leq R_{-}$ il est clair que la s\'{e}rie ne converge pas.
...
FIG. 2.2-Illustration d'un anneau de convergence
...
&=\ a\ X(z)+bY(z) 
\end{align*}
La r\'{e}gion de convergence est au moins l'intersection des r\'{e}gions associ\'{e}es \`{a} $X(z)$ et $Y(z)$ car la combinaison lin\'{e}aire peut introduire des z\'{e}ros qui com- pensent certains p\^{o}les.
...
%$$
%$$
%=\ x(-1)+z^{-1}X_{u}(z)\text{   }(2.54)
%$$
%On voit donc qu'un d\'{e}calage vers la droite permet de faire appara\^{i}tre des \'{e}l\'{e}- ments qui ne sont pas pris en consid\'{e}ration par la transformation de d\'{e}part. Cette propri\'{e}t\'{e} permet de tenir compte de conditions initiales non nulles.
%
...
%y(n)=x(n)+ay(n-1)\ (2.55)
%$$
%avec $y(-1)=K$ et une excitation $x(n)=\rme^{2 \rmi \pi \nu_0 n}u(n)$ . En vertu de 2.54,
%
%la transform\'{e}e en $z$ donne
%$$
%Y_{u}(z)\ =\ X_{u}(z)+ay(-1)+az^{-1}X_{u}(z)
%$$
%$$
%=\ \frac{X_{u}(z)+ay(-1)}{1-az^{-1}}
%$$
%$$
%=\ \frac{aK}{1-az^{-1}}+\frac{1}{(1-az^{-1})(1-e^{j\Omega_{0}}z^{-1})}
%$$
%$$
%=\ \frac{aK}{1-az^{-1}}+\frac{1}{(1-e^{j\Omega_{0}}/a)(1-az^{-1})}
%$$
%1
%$$
%+
%$$
%$$
%(1-a/e^{j\Omega_{0}})(1-e^{j\Omega_{0}}z^{-1})
%$$
%$$
%=\ \frac{aK}{1-az^{-1}}+\frac{a}{(a-e^{j\Omega_{0}})(1-az^{-1})}
%$$
%$$
%+\ \frac{e^{j\Omega_{0}}}{(e^{j\Omega_{0}}-a)(1-e^{j\Omega_{0}}z^{-1})}\ (2.56)
%$$
%De ce fait, en utilisant les transform\'{e}es inverses d\'{e}j\`{a} \'{e}tablies, on
%
%trouve
%$$
%y(n)=u(n)[a^{n+1}K+\frac{a^{n+1}}{a-e^{j\Omega_{0}}}-\frac{e^{j\Omega_{0}(n+1)}}{a-e^{j\Omega_{0}}}]\ (2.57)
%$$
...
%r\'{e}ponse forc\'{e}e.
%
%On peut bien \'{e}videmment g\'{e}n\'{e}raliser ce qui pr\'{e}c\`{e}de au cas de plusieurs condi- tions initiales.
%
...
%Comme
%$$
%X(z)=\sum_{n=-\infty}^{\infty}x(n)z^{-n}\text{   }(2.60)
%$$
%on a aussi
%$$
%\frac{\mathrm{d}X(z)}{\mathrm{d}z}=\sum_{n=-\infty}^{\infty}(-n)x(n)z^{-n-1}\text{   }(2.61)
%$$
%Et donc
%$$
%-z\frac{\mathrm{d}X(z)}{\mathrm{d}z}=\sum_{n=-\infty}^{\infty}nx(n)z^{-n}\text{   }(2.62)
%$$
%De ce fait, il appara\^{i}t $\mathrm{q}\mathrm{u}\mathrm{e}-z\mathrm{d}X(z)/\mathrm{d}z$ est la transform\'{e}e de $nx(n)$ .
%
%Convolution
%
%Cette propri\'{e}t\'{e} est une des plus importantes et justffie \`{a} elle seule l'usage qui est fait de la transform\'{e}e en $z$ pour \'{e}tudier les syst\`{e}mes lin\'{e}aires permanents en temps discret. Si $y(n)$ est obtenu par convolution de $x(n)$ et $g(n)$ , on a que
%$$
%y(n)=\sum_{m=-\infty}^{\infty}x(m)\ g(n-m)\text{   }(2.63)
%$$
%{\it 2.2. LA TRANSFORM\'{E}E EN} $Z$
%
%27
%
%La transform\'{e}e en $z,\ Y(z)$ , de $y(n)$ est donc obtenue par
%$$
%Y(z)\ =\ \sum_{n=-\infty}^{\infty}y(n)z^{-n}
%$$
%$$
%=\ \sum_{n=-\infty}^{\infty}\sum_{m=-\infty}^{\infty}x(m)\ g(n-m)z^{-n}
%$$
%$$
%=\ [\sum_{m=-\infty}^{\infty}x(m)z^{-m}][\sum_{n=-\infty}^{\infty}g(n-m)z^{-(n-m)}]
%$$
%$$
%=\ X(z)G(z)\text{   }(2.64)
%$$
...
%
%Consid\'{e}rons un syst\`{e}me lin\'{e}aire et permanent de r\'{e}ponse impul- sionnelle
%$$
%g(n)=a^{n}u(n)\ (2.65)
%$$
%et $|a|<1$ dont le signal $x(n)$ \`{a} l'entr\'{e}e est la suite
%$$
%x(n)=b^{n}u(n)\ (2.66)
%$$
%et $|b|<1$ avec de plus $|b|>|a|$. La sortie $y(n)$ est obtenue par $y(n)=x(n)\otimes g(n)$ o\`{u} $\otimes$ est le symbole utilis\'{e} pour une convolution. Les transform\'{e}es $X(z)$ et $G(z)$ sont donn\'{e}es par
%
%$X(z) = \displaystyle \frac{1}{1-bz^{-1}}$ pour $|z|>|b|$ (2.67) $G(z) = \displaystyle \frac{1}{1-az^{-1}}$ pour $|z|>|a|$ (2.68)
%
...
%$$
%$$
%=\ \frac{1}{1-az^{-1}}\frac{1}{1-bz^{-1}}\ (2.70)
%$$
%$$
%=\ \frac{1}{(1-b/a)(1-az^{-1})}+\frac{1}{(1-a/b)(1-bz^{-1})}(2.71)
%$$
%$$
%=\ \frac{a}{(a-b)(1-az^{-1})}+\frac{b}{(b-a)(1-bz^{-1})}\ (2.72)
%$$
%et donc, compte tenu des transform\'{e}es calcul\'{e}es pr\'{e}c\'{e}demment,
%$$
%y(n)=u(n)[\frac{a}{(a-b)}a^{n}+\frac{b}{(b-a)}b^{n}]\ (2.73)
%$$
