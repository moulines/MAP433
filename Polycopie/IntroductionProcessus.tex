...
%D'autres s\'eries de ce type peuvent être obtenues sur
%\emph{The Internet Traffic Archive},
%\emph{http://ita.ee.lbl.gov/}.
%%======== FIGURE
% \figscale{\FIGPASSL lbl_tcp_3}
% {Trace de trafic Internet~: temps d'inter-arriv\'ees de paquets TCP.}
% {fig:figtraf}{0.5}
%\end{example}
...
%======== FIGURE
 ...
 {Signal de parole \'echantillonn\'e \`a $8000$ Hz~:
 son non vois\'e \emph{ch}.}{fig:figspeech}{1}
\end{example}
...
%======== FIGURE
 ...
 entre Janvier 1990 et Ao\^ut 2000.}{fig:SP}{1}
\end{example}
...
On d\'efinit de même un espace mesurable produit $(E_1\times\dots\times E_n,\cE_1\otimes\dots\otimes\cE_n)$
...
Le th\'eor\`eme~\ref{th:kolmogorov} peut aussi être utilis\'e pour le
...
d\'etaill\'ee peut être trouv\'ee dans~\cite[Chapter~16]{jacod:protter:2003}.
...
En utilisant le même proc\'ed\'e de preuve que pour la
...
n'a donc un int\'erêt que dans le cas o\`u $T$ est de cardinal infini.
...
%paraît~: si {\em la loi} du processus est bien d\'efinie de mani\`ere unique,
...
%mêmes lois, et la loi de l'incr\'ement $X(t) - X(s)$ est de
...
%nombre de variables al\'eatoires ind\'ependantes de même loi
...
% \clnote{pourquoi n'\'ecrit-on pas cette d\'efinition pour la stationnarit\'e
%   stricte ?}
% \begin{definition}[Stationnarit\'e stricte]
% Un processus al\'eatoire $(X_t)_{t\in\Zset}$ est stationnaire au sens
...
Un processus al\'eatoire $\{X_t, t\in T \}$ est stationnaire au sens strict si $X$ et $S\circ X$ ont même loi, \ie\
 $\PP_{S\circ X}=\PP_X$.
\end{definition}
...
\PP_I=\PP_{I+1} \; .
$$
On remarque aussi que la stationnarit\'e au sens strict implique que  $X$ et $S^\tau\circ X$ ont même loi pour tout $\tau\in T$
...
peuvent être utilis\'es pour approcher une grande classe de processus d\'ependants
...
filtrage doit être d\'efinie avec quelques pr\'ecautions. Il faut en effet tout
