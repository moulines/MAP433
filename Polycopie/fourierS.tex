...
\begin{example}
  Il est facile de v{\'e}rifi{\'e} que la fonction $f$ d{\'e}finie par $f(x)=\rme^{-x^2}$
...
d{\'e}finie par
$$
f \star g(x) = \int f(x - t)g(t) dt = \int f(u)g(x - u) \rmd u\;,
$$
...
$$
\int f (x - t)g(t) dt = \int_0^1 \1_{[0,1]}(x - t) dt = \mathrm{Leb} ([0,1] \cap [x - 1,x]).
$$
...
Calculons $f \star g$.
\begin{equation}
f * g(x) = \frac{1}{2h} \int_{x-h}^{x+h}  f (u) du
\end{equation}
...
\int f(t-u) K_\sigma(u) du - f(t) = \int \left\{ f(t - u) - f(t) \right \} K_\sigma(u) du \eqsp.
\end{equation}
...
\begin{equation}
\label{eq:conv-local}
\int_{|u| \leq \eta} \left|  f(t-u) - f(t) \right | K_\sigma(u) du \leq \epsilon \int K_\sigma(u) du = \epsilon \eqsp.
\end{equation}
De plus,
$$
\int_{|u| \geq \eta} \left| f(t-u) - f(t) \right | K_\sigma(u) du  \leq \|f\|_1\,\sup_{|u|\geq\eta} {K}_\sigma(u)
+|f(t)|\,\int_{|u|\geq \eta} {K}_\sigma(u) du  \eqsp.
$$
...
et,
$$
\int_{u\geq \eta} K_\sigma(u) du \leq \frac{C}\sigma\int_{|v|\geq\eta/\sigma} (1+|v|)^{-\alpha}\,dv =O(\sigma^{\alpha-2})\eqsp.
$$
Par cons{\'e}quent, en combinant ces majorations avec $\alpha>2$,
\begin{equation}
\label{eq:conv-hors-local}
\lim_{\sigma \downarrow 0} \int_{|u| \geq \eta} \left|  f(t-u) - f(t) \right | K_\sigma(u) du =0 \eqsp.
\end{equation}
...
$f$ et $\rme^{\rmi 2 \pi t x} \hat{g}_\sigma(x)$ donne donc, pour tout $t\in \rset$,
\begin{equation}
\label{eq:inversionL1-1}
\int \hat{f}(x) \hat{g}_\sigma(x) \rme^{\rme 2 \pi t x} \rmd x = \int f(u) {g}_\sigma(t-u) du = f\star g_\sigma(t) \eqsp.
\end{equation}
Lorsque $\sigma \to 0$, on peut passer {\`a} la limite dans l'int{\'e}grale de gauche, puisque l'on a, pour tout
$x$, $\lim_{\sigma \to 0} g_\sigma(x)= 1$ pour tout $x$ et $|\hat{f}(x) g_\sigma(x) \rme^{\rmi 2 \pi t x}| \leq |\hat{f}(x)|$.
Comme $\hat{f} \in L_1(\rset)$, on applique le th{\'e}or{\`e}me de convergence domin{\'e}e, qui montre
$$
\lim_{\sigma \to 0} \int \hat{f}(x) g_\sigma(x) \rme^{ \rmi 2 \pi t x} \rmd x = \int \hat{f}(x) \rme^{\rmi 2 \pi t x} \rmd x \eqsp.
$$
...
$g=0$. De plus, d'apr{\`e}s le th{\'e}or{\`e}me~\ref{thm:densCL1}, on peut se contenter de prendre $f$ continue, le cas g{\'e}n{\'e}ral {\'e}tant
...
\section{Compl{\'e}ment: petit retour {\`a} la convolution}
...
$$
\iint f(y) g(z) d y d z = \iint f(x-t) g(t) \rmd x dt \eqsp
$$
et les int{\'e}grales des modules sont finies.
La fonction $x \mapsto \int f(x-t) g(t) dt$ est donc d{\'e}finie presque partout et appartient {\`a} $L_1(\rset)$, toujours d'apr{\`e}s le th{\'e}or{\`e}me de Fubini.
La seconde in{\'e}galit{\'e} d{\'e}coule de:
$$
\int | f \star g(x)| \rmd x = \int |g(t)| \left( \int  |f(x-t) \rmd x \right) dt = \|f\|_1 \| g \|_1 \eqsp.
$$
