...
%   anglais) comme l'op\'erateur de l'espace $\cH^X_\infty$ dans lui-même
...
%   si $B$ est une isom\'etrie de $\cH^X_\infty$ dans lui-même.
% \end{proposition}
...
$$
%\clnote{j'ai pr\'ecis\'e la notation de la limite croissante}
...
lui-même stationnaire au second ordre, \`a condition que la
...
%L'int\'erêt de cette param\'etrisation est qu'\`a partir d'un nombre fini de
...
\[
  X_t = Z_t + \phi_1 \sum_{k=0}^{+\infty} \phi_1^k Z_{t-1-k}
      = Z_t + \phi_1 X_{t-1}\;.
\]
Remarquons que la solution donn\'ee par \eqref{AR1sol} peut être obtenu
...
valeurs de $\phi_1$ r\'eelles et n\'egatives conduisent \`a des trajectoires o\`u une valeur positive a tendance \`a être
...
\end{eqnarray*}
En utilisant exactement les mêmes arguments que ceux employ\'es
...
\clnote{je ne comprends pas le 2\`eme point}
\end{enumerate}
Du point de vue de la mod\'elisation, la pr\'esence de z\'eros communs
ne pr\'esente aucun int\'erêt puisqu'elle est sans influence sur
la densit\'e spectrale de puissance. Elle conduit de plus \`a une
ambiguït\'e sur l'ordre r\'eel des parties AR et MA.
%=========================================================
...
% mod\`ele ARMA$(p,q)$ lui-même et pas uniquement au processus
% $X_t$.
%\begin{exercice}
...
%=======================================================================================
%=======================================================================================
% \section{Preuves des th\'eor\`emes~\ref{theo:filtragepassl} et~\ref{theo:filtragepassl_stat}}
% \label{sec:c1preuves}
%=======================================================================================
...
% al\'eatoire tel que $\sup_{t \in \Zset} \PE {|X_{t}|} < \infty$.
% Alors, pour tout $t \in \Zset$, la suite~:
% \[
...
% quadratique vers $Y_{t}$, c'est \`a dire que $\limn \PE {|Y_{n,t} -
% Y_{t}|^2} = 0$.
% \end{theorembis}
%===============================================================
...
% \end{equation}
% o\`u $\psi(e^{-i\lambda}) = \sum_k \psi_k e^{-i k \lambda}$ est
